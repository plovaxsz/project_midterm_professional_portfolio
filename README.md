...
Table of Contents
    ...
    Setup & Configuration: A Step-by-Step Walkthrough
...
The core library from Facebook for building the entire User Interface (UI) using a declarative, component-based model.
	

Sass (SCSS)
	

A powerful CSS preprocessor that allows for more maintainable and organized styling through variables, mixins, and nesting.
	

Framer Motion
	

A production-ready animation library that brings the UI to life with fluid transitions and interactive effects.
...
Acts as the project's brain. It's a cloud-hosted database and a customizable content editor called Sanity Studio.
...
The crucial communication layer. Our React app uses this API to send requests (e.g., "get all projects") to the Sanity backend.
...
The hosting platform for our frontend. It connects to GitHub and automatically deploys any new changes.
	

Sanity.io Platform
	

Provides hosting for the backend database and the online version of the Content Studio.
...
Git is used for version control locally. GitHub stores the code remotely and acts as the source for Vercel's deployments.
...
Creates isolated, self-contained "boxes" (containers) for our frontend and backend to run in, avoiding local setup conflicts.
	

Docker Compose
	

The "conductor" for Docker. It reads a YAML file to manage and run our multi-container application with a single command.
...
git clone https://github.com/plovaxsz/project_midterm_professional_portfolio.git
...
The frontend application needs to know where to find the Sanity.io backend and how to authenticate with it. We provide this information via an environment variables file (.env). This file is intentionally not tracked by Git (.gitignore) to keep sensitive keys private.
    ...
    # This is a secret API token used to authenticate requests. It allows the React app to read data from a private Sanity dataset.
...
Bash
    ...
    Leave this terminal window open and running as well.

6. Accessing the Running Application ✅

Once the setup is complete and the servers are running, the application will be accessible at the following localhost addresses in your web browser:
